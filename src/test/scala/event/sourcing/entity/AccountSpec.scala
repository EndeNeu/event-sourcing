...
class AccountSpec extends WordSpecLike with Matchers with CommonSpec {
  ...
  "An account" should {
    "correctly parse commands" in new TestContext {
      val account = AccountService.openAccount(AccountOpenCommand(UUID.randomUUID(), 300))
      ...
      val poorAccount = AccountService.openAccount(AccountOpenCommand(UUID.randomUUID(), 0))
    ...
    "correctly parse events" in new TestContext {
      val account = AccountService.openAccount(AccountOpenCommand(UUID.randomUUID(), 0))
